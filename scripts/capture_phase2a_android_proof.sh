...
ARTIFACT_DIR="${ROOT_DIR}/phases/2a-outbound-sms/artifacts/android-proof"
PACKAGE_NAME="com.freeline.app"
ACTIVITY_NAME="com.freeline.app/.MainActivity"
APK_PATH="${ANDROID_DIR}/app/build/outputs/apk/debug/app-debug.apk"
AVD_NAME="${ANDROID_PROOF_AVD:-Pixel_6a}"
EMULATOR_LOG="/tmp/freeline_phase2a_android_emulator.log"
EMULATOR_STARTED=0
EMULATOR_PID=""
...
sleep 3
adb -s "${ANDROID_SERIAL}" exec-out screencap -p >"${ARTIFACT_DIR}/thread-flow-inbox.png"

adb -s "${ANDROID_SERIAL}" shell am force-stop "${PACKAGE_NAME}" >/dev/null
adb -s "${ANDROID_SERIAL}" shell am start \
  -n "${ACTIVITY_NAME}" \
  --es proofScenario "thread-send" >/dev/null
sleep 5.5
adb -s "${ANDROID_SERIAL}" exec-out screencap -p >"${ARTIFACT_DIR}/thread-flow-sent.png"

adb -s "${ANDROID_SERIAL}" shell am force-stop "${PACKAGE_NAME}" >/dev/null
adb -s "${ANDROID_SERIAL}" shell am start \
  -n "${ACTIVITY_NAME}" \
  --es proofScenario "compose-send" >/dev/null
sleep 2.5
adb -s "${ANDROID_SERIAL}" exec-out screencap -p >"${ARTIFACT_DIR}/compose-flow-compose.png"
sleep 3
adb -s "${ANDROID_SERIAL}" exec-out screencap -p >"${ARTIFACT_DIR}/compose-flow-sent.png"

printf 'Saved phase 2a Android proof artifacts to %s\n' "${ARTIFACT_DIR}"
