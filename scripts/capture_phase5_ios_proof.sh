...
ARTIFACT_DIR="${ROOT_DIR}/phases/5-ads/artifacts/ios-proof"
DERIVED_DATA="${ROOT_DIR}/.runtime/ios-phase5-proof"
...
for scenario in "${SCENARIOS[@]}"; do
  xcrun simctl terminate "${DEVICE_UDID}" "${BUNDLE_ID}" >/dev/null 2>&1 || true
  xcrun simctl launch "${DEVICE_UDID}" "${BUNDLE_ID}" -proofScenario "${scenario}" >/dev/null
  sleep "$(scenario_wait_seconds "${scenario}")"
  xcrun simctl io "${DEVICE_UDID}" screenshot "${ARTIFACT_DIR}/${scenario}.png" >/dev/null
done

printf 'Saved phase 5 iOS proof artifacts to %s\n' "${ARTIFACT_DIR}"
