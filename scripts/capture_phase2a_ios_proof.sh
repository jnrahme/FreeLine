...
ARTIFACT_DIR="${ROOT_DIR}/phases/2a-outbound-sms/artifacts/ios-proof"
DERIVED_DATA="${ROOT_DIR}/.runtime/ios-phase2a-proof"
...
sleep 2.5
xcrun simctl io "${DEVICE_UDID}" screenshot "${ARTIFACT_DIR}/thread-flow-inbox.png" >/dev/null

xcrun simctl terminate "${DEVICE_UDID}" "${BUNDLE_ID}" >/dev/null 2>&1 || true
xcrun simctl launch "${DEVICE_UDID}" "${BUNDLE_ID}" -proofScenario thread-send >/dev/null
sleep 5.5
xcrun simctl io "${DEVICE_UDID}" screenshot "${ARTIFACT_DIR}/thread-flow-sent.png" >/dev/null

xcrun simctl terminate "${DEVICE_UDID}" "${BUNDLE_ID}" >/dev/null 2>&1 || true
xcrun simctl launch "${DEVICE_UDID}" "${BUNDLE_ID}" -proofScenario compose-send >/dev/null
sleep 2.5
xcrun simctl io "${DEVICE_UDID}" screenshot "${ARTIFACT_DIR}/compose-flow-compose.png" >/dev/null
sleep 3
xcrun simctl io "${DEVICE_UDID}" screenshot "${ARTIFACT_DIR}/compose-flow-sent.png" >/dev/null

printf 'Saved phase 2a iOS proof artifacts to %s\n' "${ARTIFACT_DIR}"
